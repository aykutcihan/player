...
export default function NowBar({ channel, visible }: Props) {
  ...
  // Kanal değişince scroll sıfırla
  useEffect(() => {
  ...
  // Current yüklenince logonun yanına scroll et
  useEffect(() => {
    if (!current) return
    const go = () => {
  ...
  const onMouseUp = () => {
    ...
    // moved'ı bir sonraki frame'de sıfırla — click event'i önce ateşlensin
    setTimeout(() => { dragStart.current.moved = false }, 0)
  }

  const handleClick = (prog: Programme) => {
    // Drag sırasında click'i engelle
