...
export default function Layout() {
  ...
  useEffect(() => {
    // Capacitor back button — normal kısa basış (tek sayfalı uygulama: kısa basış no-op)
    const handler = App.addListener('backButton', () => {
      backButtonBus.handle()
    })
    ...
    // Web/TV klavye geri (ESC, Samsung Back) — tek sayfalı uygulama: yalnızca açık katmanları kapatır
    const onKey = (e: KeyboardEvent) => {
      if (e.keyCode === 27 || e.keyCode === 10009) backButtonBus.handle()
    }
